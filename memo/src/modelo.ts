...
const crearColeccionDeCartasInicial = (infoCartas: InfoCarta[]): Carta[] => {
  let arrayCartasInicial: Carta[] = [];
  infoCartas.map((carta) => {
    const cartaInicial = crearCartaInicial(carta.idFoto, carta.imagen);
    arrayCartasInicial = [...arrayCartasInicial, cartaInicial, cartaInicial];
  });
  return arrayCartasInicial;
};
